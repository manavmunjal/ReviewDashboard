...
public class ReviewServiceTest {
    ...
    public void testAddReview() {
        ...
        ReviewDTO result = reviewService.addReview(productId, review);
    ...
    public void testGetAverageRating() {
        ...
        ResponseEntity<Double> response = ResponseEntity.ok(expectedRating);

        // Mock the reviewClient to return the response entity
        when(reviewClient.getAverageRating(anyString())).thenReturn(response);

        // When: Call the method under test
        ResponseEntity<Double> result = reviewService.getAverageRating(productId);

        // Then: Verify the result
        assertEquals(response, result, "The response entity should match the one from the client.");
        assertEquals(expectedRating, result.getBody(), "The rating in the body should match the expected rating.");
    }
}
